...
fn run_both(n: usize, steps: u32) {
    ...
    for t in 1..=steps {
        ...
        metal.step(
            &mut param_gpu, &grad, &mut m_gpu, &mut v_gpu,
            t, cfg.lr, cfg.beta1, cfg.beta2, cfg.eps, cfg.weight_decay,
        );

        // Check agreement after each step
        let max_param_diff = param_cpu.iter().zip(&param_gpu)
            .map(|(a, b)| (a - b).abs())
            .fold(0.0f32, f32::max);
        let max_m_diff = m_cpu.iter().zip(&m_gpu)
            .map(|(a, b)| (a - b).abs())
            .fold(0.0f32, f32::max);
        let max_v_diff = v_cpu.iter().zip(&v_gpu)
            .map(|(a, b)| (a - b).abs())
            .fold(0.0f32, f32::max);

        // Allow small float precision differences (GPU uses fma, CPU uses vDSP)
        let tol = 1e-5;
        assert!(max_param_diff < tol, "step {t}: param diff {max_param_diff}");
