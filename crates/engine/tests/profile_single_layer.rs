...
use engine::layer::{self, CompiledKernels, LayerWeights, LayerGrads};
...
fn profile_layer_ops() {
    ...
    let stage_fwd = fwd_t.stage_sdpa_ms + fwd_t.stage_wo_ms + fwd_t.stage_ffn_ms
        + fwd_t.read_sdpa_ms + fwd_t.read_wo_ms + fwd_t.read_ffn_ms;
    let cpu_fwd = fwd_t.rmsnorm1_ms + fwd_t.residual_rmsnorm2_ms;

    let ane_bwd = bwd_t.stage_run_ffn_bwd_w2t_ms + bwd_t.async_ffn_bwd_w13t_plus_dw_ms
        + bwd_t.stage_run_wot_bwd_ms + bwd_t.async_sdpa_bwd1_plus_dwo_ms
        + bwd_t.stage_run_sdpa_bwd2_ms + bwd_t.async_q_bwd_plus_dw_ms
        + bwd_t.stage_run_kv_bwd_ms;
    let cpu_bwd = bwd_t.scale_dy_ms + bwd_t.silu_deriv_ms + bwd_t.rmsnorm2_bwd_ms
        + bwd_t.rope_bwd_ms + bwd_t.rmsnorm1_bwd_ms + bwd_t.merge_dx_ms;
    let stage_bwd = bwd_t.stage_ffn_bwd_w13t_ms + bwd_t.stage_sdpa_bwd1_ms
        + bwd_t.stage_q_bwd_ms + bwd_t.read_sdpa_bwd1_ms;

    println!("  FORWARD:  ANE {ane_fwd:.1}ms | staging {stage_fwd:.1}ms | CPU {cpu_fwd:.1}ms | total {:.1}ms", fwd_t.total_ms);
    println!("  BACKWARD: ANE+dW {ane_bwd:.1}ms | staging {stage_bwd:.1}ms | CPU {cpu_bwd:.1}ms | total {:.1}ms", bwd_t.total_ms);
