//! Correctness test for 4-thread parallel Adam update (optimized for 1024 model).
//!
//! What was optimized:
//!   Changed update_weights from 3-thread to 4-thread split. For 8 layers,
//!   old split was (3,3,2) with Thread 1 also handling embed+gamma → 46.9M bottleneck.
//!   New split is (2,2,2,2) → 34.1M bottleneck (27% reduction in worst-thread work).
//!
//! Invariant checked:
//!   4-thread parallel update_weights must produce bit-exact results vs sequential
//!   step_fused for both 6-layer (gpt_karpathy) and 8-layer (gpt_1024) configs.
//!   step_fused is element-wise on disjoint memory; thread assignment doesn't affect output.
//!
//! Tolerance: 0.0 (exact match — no math change, only different thread split)
//!
//! Failure meaning:
//!   A data race or incorrect memory split exists in the 4-thread implementation.
...
/// Test 1: 4-thread Adam matches sequential for gpt_1024 (8 layers) — the target config.
/// 4-way split: (2,2,2,2) with Thread 1 also handling embed+gamma.
#[test]
fn adam_4thread_matches_sequential_1024() {
    ...
    compare_weights(&w_seq, &w_par, "1024_step1");
    println!("PASS: 4-thread Adam matches sequential for gpt_1024 (8 layers, ~111M params)");
}

/// Test 2: 4-thread Adam still works for gpt_karpathy (6 layers) — regression check.
/// 4-way split: (1,1,2,2) with Thread 1 also handling embed+gamma.
#[test]
fn adam_4thread_matches_sequential_768() {
    ...
    compare_weights(&w_seq, &w_par, "768_step1");
    println!("PASS: 4-thread Adam matches sequential for gpt_karpathy (6 layers, ~48.8M params)");
}

/// Test 3: 5 steps with gpt_1024 — catch accumulation drift over multiple steps.
#[test]
fn adam_4thread_1024_matches_5_steps() {
    let cfg = ModelConfig::gpt_1024();
    let tc = TrainConfig::default();
    let metal = MetalAdam::new().expect("Metal required");
    ...
    for t in 1u32..=5 {
        ...
        compare_weights(&w_seq, &w_par, &format!("1024_step{t}"));
    }
    println!("PASS: 4-thread Adam matches sequential over 5 steps with gpt_1024");
}
