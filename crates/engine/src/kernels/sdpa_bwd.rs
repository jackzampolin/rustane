...
pub fn build_bwd1(cfg: &ModelConfig) -> Graph {
    ...
    let in_ch = 4 * q_dim;
    let mut g = Graph::new();
    let input = g.placeholder(Shape { batch: 1, channels: in_ch, height: 1, width: seq });
    ...
    let q4 = g.reshape(q_flat, Shape { batch: 1, channels: heads, height: hd, width: seq });
    let k4 = g.reshape(k_flat, Shape { batch: 1, channels: heads, height: hd, width: seq });
    let v4 = g.reshape(v_flat, Shape { batch: 1, channels: heads, height: hd, width: seq });
    let da4 = g.reshape(da_flat, Shape { batch: 1, channels: heads, height: hd, width: seq });
    ...
    let dv_flat = g.reshape(dvt, Shape { batch: 1, channels: q_dim, height: 1, width: seq });

    // Flatten probs: [1, HEADS, SEQ, SEQ] → [1, SCORE_CH, 1, SEQ]
    let probs_flat = g.reshape(probs, Shape { batch: 1, channels: score_ch, height: 1, width: seq });

    // Flatten dp: [1, HEADS, SEQ, SEQ] → [1, SCORE_CH, 1, SEQ]
    let dp_flat = g.reshape(dp, Shape { batch: 1, channels: score_ch, height: 1, width: seq });
...
pub fn build_bwd2(cfg: &ModelConfig) -> Graph {
    ...
    let probs = g.reshape(probs_flat, Shape { batch: 1, channels: heads, height: seq, width: seq });
    let dp = g.reshape(dp_flat, Shape { batch: 1, channels: heads, height: seq, width: seq });
    ...
    let dq_flat = g.reshape(dqt, Shape { batch: 1, channels: q_dim, height: 1, width: seq });
    let dkt = g.transpose(dk4, [0, 1, 3, 2]);
    let dk_flat = g.reshape(dkt, Shape { batch: 1, channels: q_dim, height: 1, width: seq });
